...
#### 1. キャッシュ描画 (Stamps)
花の形状を事前にオフスクリーンキャンバスに描画（スタンプ化）し、毎フレームの描画命令発行コストを大幅に削減しました。

#### 2. 静的レイヤー (Static Layering)
成長しきった花や変化しない要素を静的なキャンバス（`status-flowers-static`）に焼き付け、毎フレームの再描画を回避。これにより、数千個のパーティクルが存在しても高パフォーマンスを維持します。

#### 3. FPS制限
アニメーションループを30FPSに制限し、モバイルデバイスでのバッテリー消費とCPU使用率を抑制しました。
